...
void UAirBlueprintLib::writeLogMessage(const std::string& message, float x, float y, float z, float roll, float pitch, float yaw)
{
    time_t rawtime;
    struct tm* timeinfo;
    bool append = true;
    std::string logText = "\"" + message + "\"";
    // Write log to UE_4.24\Engine\Binaries\Win64\<FilePath>
    std::string filePath = "GameLog.txt";

    //Remove semicolons from message
    int pos = logText.find(';');
    ...
    logText = logText + ";" + std::to_string(x) + ";" + std::to_string(y) + ";" + std::to_string(z) + ";" + std::to_string(roll) + ";" + std::to_string(pitch) + ";" + std::to_string(yaw);

    if (!append) {
        //Write format of file for access import
        std::string format = "\"Date\";\"Time\";\"Object\";\"X\";\"Y\";\"Z\";\"Roll\";\"Pitch\";\"Yaw\"";
        std::ofstream myfile;
    ...
    char buf[256];
    strftime(buf, 256, "%m/%d/%g;%H:%M:%S;", timeinfo);
    char* msg = buf;

    // Add timestamp to entry
    strcat(msg, logText.c_str());

    // Add checksum to entry
    // Checksum = (Hours + Minutes + Seconds) * (Month + Day + Year - Seconds)) / (SumOfMessageAscii + 993319.0 + messageField.length)
    std::string logMessage = std::string(msg);


    std::string month = logMessage.substr(0, 2);
    std::string day = logMessage.substr(3, 2);
    std::string year = logMessage.substr(6, 2);

    std::string hours = logMessage.substr(9, 2);
    std::string minutes = logMessage.substr(12, 2);
    std::string seconds = logMessage.substr(15, 2);

    int beg = logMessage.find("\"");
    int end = logMessage.find("\"", beg + 1) - 1;
    int messageFieldLength = end - beg;
    std::string LogText = logMessage.substr(beg, messageFieldLength);

    int sum = 0;
    for (int i = 0; i < messageFieldLength; i++) {
        sum += LogText[i];
    }

    float checksum = -1;
    try {
        checksum = (std::stoi(month) + std::stoi(day) + std::stoi(year) - std::stoi(seconds)) * (std::stoi(hours) + std::stoi(minutes) + std::stoi(seconds)) / (float)(sum + 993319.0 + messageFieldLength);
    }
    catch (...) {
    ...
    strcat(msg, ";");
    strcat(msg, std::to_string(checksum).c_str());

    std::ofstream myfile;
    myfile.open(filePath, std::ios::app | std::ios::out);
