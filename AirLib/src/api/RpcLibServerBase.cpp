    ...
    : api_provider_(api_provider)
{
    ...
    pimpl_->server.bind("writeLogMessage", [&](const std::string& message, float x, float y, float z, float roll, float pitch, float yaw) -> void {
        getWorldSimApi()->writeLogMessage(message, x, y, z, roll, pitch, yaw);
        });
